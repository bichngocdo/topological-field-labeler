...
import seqlbl.model.data_reader as data_reader
import seqlbl.model.data_writer as data_writer
from seqlbl.data.bucketing import Bucketing
from seqlbl.data.dataset import Dataset, SimpleBatch
from seqlbl.model.data_converter import DataConverter
from seqlbl.model.data_loader import DataLoader
from seqlbl.model.evaluator import Stats
from seqlbl.model.network import SequenceLabeler
from seqlbl.utils.embeddings import read_embeddings
...
class Experiment(object):
    ...
    def init_data_model(self, raw_data):
        ...
        self.args.no_labels = len(self.data_loader.label_vocab)
    ...
    def init_network(self, session):
        self.network = SequenceLabeler(self.args)
        self.network.initialize_global_variables(session)
    ...
    def init_evaluators(self):
        ignore_labels = {self.data_loader.NONE, self.data_loader.UNKNOWN}
        ignore_label_ids = set([self.data_loader.label_vocab[label] for label in ignore_labels])

        main_labels = set([label for label in self.data_loader.label_vocab.str2id]) - ignore_labels - {'NULL'}
        main_label_ids = set(self.data_loader.label_vocab[label] for label in main_labels)

        self.train_stats = Stats('train',
                                 main_labels=main_label_ids,
                                 ignore_labels=ignore_label_ids)
        self.dev_stats = Stats('dev',
                               main_labels=main_label_ids,
                               ignore_labels=ignore_label_ids)

        print('Main labels:', ', '.join(main_labels))
        print('Ignore labels:', ', '.join(ignore_labels))
    ...
    def convert_results(self, outputs):
        ...
        for output in outputs:
            ...
            for id in output:
                if id >= 0:
                    labels.append(self.data_loader.label_vocab.lookup(id))
    ...
    def __validate(self, dev_dataset, eval_batch_size, session):
        ...
        summary = self.network.make_train_summary(session, stats['train_f1'], stats['train_acc'])
        self.summary_writer.add_summary(summary, global_step=self.iteration)
        summary = self.network.make_dev_summary(session, stats['dev_loss'], stats['dev_f1'], stats['dev_acc'])
        self.summary_writer.add_summary(summary, global_step=self.iteration)

        self.save()
        self.save_network(session)

        if stats['dev_f1'] > self.best_score:
            self.best_score = stats['dev_f1']
...
def train(args):
    ...
    train_raw_data = data_reader.read(args.train_file, args.input_cols)
    print('Load data from file %s, no. sentences: %d' % (args.train_file, len(train_raw_data[0])))

    dev_raw_data = data_reader.read(args.dev_file, args.input_cols)
    print('Load data from file %s, no. sentences: %d' % (args.dev_file, len(dev_raw_data[0])))

    if args.test_file is not None:
        test_raw_data = data_reader.read(args.test_file, args.input_cols)
        print('Load data from file %s, no. sentences: %d' % (args.test_file, len(test_raw_data[0])))
    else:
        test_raw_data = None
...
def evaluate(args):
    ...
    dev_raw_data = data_reader.read(args.test_file, args.input_cols)
    print('Load data from file %s, no. sentences: %d' % (args.test_file, len(dev_raw_data[0])))
    ...
    if args.output_file is not None:
        data_writer.write(args.output_file, dev_raw_data, results)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(prog='experiment.py',
                                     description='NN Sequence Labeling')
    ...
    group.add_argument('--word_cutoff', type=int, default=0,
                       help='word cutoff threshold')
    group.add_argument('--lowercase', action='store_true',
                       help='lowercasing words')
    group.add_argument('--input_cols', type=int, nargs='+', default=[0, 1, 2],
                       help='input columns')

    # Training parameters
    group = parser_train.add_argument_group('training parameters')
    ...
    group.add_argument('--train_batch_size', type=int, default=512,
                       help='mini-batch size in number of words')
    ...
    # Network parameters
    group = parser_train.add_argument_group('network parameters')
    ...
    group.add_argument('--num_lstms', type=int, default=2,
                       help='number of LSTM layers')
    group.add_argument('--num_mlps', type=int, default=1,
                       help='number of MLP layers')
    group.add_argument('--mlp_dim', type=int, default=100,
                       help='MLP dimension')
    group.add_argument('--use_crf', action='store_true')
    ...
    group = parser_train.add_argument_group('regularization parameters')
    ...
    parser_eval.add_argument('--input_cols', type=int, nargs='+', default=[0, 1, 2],
                             help='input columns')
